...
{
    uint32_t i;
    COUNTER start_ns = SEC_TO_NANOSEC(start->tv_sec) + start->tv_nsec;

    printf("dump_timestamp_trace_array: start=%lld.%09lld stop=%lld.%09lld start_ns=%llu traces=%u bps=%llu\n",
    ...
    for (i = 0; i < trace_num; ++i) {
        ...
        printf("timestamp=%lld.%09lld, size=%llu now_ns=%llu tx_ns=%llu next_tx_ns=%llu delta=%lld bytes_sent=%llu skip=%llu\n",
